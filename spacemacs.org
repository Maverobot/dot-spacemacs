...
#+END_SRC
*** minuet-ai
Use Minuet with an OpenAI-compatible completion endpoint. The default is the local Qwen 3.6 27B =llama-server= at =http://127.0.0.1:8080/v1/chat/completions= with model alias =qwen3.6-27b=.

Override startup defaults with =MINUET_OPENAI_COMPATIBLE_ENDPOINT=, =MINUET_OPENAI_COMPATIBLE_MODEL=, or =MINUET_OPENAI_COMPATIBLE_API_KEY_SOURCE=. At runtime, use =SPC a i M= to select another endpoint/model; models are fetched automatically from the endpoint's =/models= API when available.
#+BEGIN_SRC emacs-lisp :tangle user-config.el
  (use-package minuet
    ...
    (defvar my/minuet-openai-compatible-api-key-source
      (or (getenv "MINUET_OPENAI_COMPATIBLE_API_KEY_SOURCE")
          (getenv "MINUET_QWEN_API_KEY_SOURCE")
          "local")
    ...
    (defvar my/minuet-conventional-commit-prompt
      "When completing a Git commit message, prefer Conventional Commits:
...
- If a body is useful, leave one blank line after the subject and explain why, not what.
      ...
      (when (file-exists-p plstore-file)
        (condition-case err
            (let ((store (plstore-open plstore-file)))
              (dolist (entry (plstore--get-entries store))
