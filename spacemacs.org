...
#+BEGIN_SRC emacs-lisp :tangle user-config.el
  (use-package org-gcal
    ...
    ;; Avoid hanging during token decryption by routing pinentry through Emacs minibuffer.
  ...
  ;; Auto-sync timer: schedule a fetch every 2 hours, but only execute when
  ;; Emacs has been idle for at least 10 seconds to avoid blocking the UI.
  (defvar my/org-gcal-timer nil
    "Timer for periodic org-gcal auto-sync.")

  (defun my/org-gcal-schedule-fetch ()
    "Run org-gcal fetch when Emacs is next idle for 10+ seconds."
    (run-with-idle-timer 10 nil #'my/org-gcal-fetch-safe))

  ;; Start the repeating 2-hour timer after a 30-second initial delay.
  ;; Each firing schedules an idle-time fetch rather than running inline.
  (setq my/org-gcal-timer
        (run-at-time "30 sec" (* 2 60 60) #'my/org-gcal-schedule-fetch))
