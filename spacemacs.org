...
Stable. Ranger is pinned to rolling MELPA because its current compatibility fix is
not yet available from MELPA Stable. mu4e-alert is also pinned to rolling MELPA
because MELPA Stable 1.0 reads the removed =mu4e~context-current= variable;
rolling 20251022.2130 supports mu4e 1.7+ APIs and passes the installed mu4e
1.10.8 context-switch repro. evil-org is pinned because MELPA Stable 1.0.2 calls
the removed =evil-redirect-digit-argument= function; rolling 20221001.2335 uses
Evil's current minor-mode keymap API and matches the build that worked with the
previous Emacs 30.2 package tree.
#+BEGIN_SRC emacs-lisp :tangle user-init.el
