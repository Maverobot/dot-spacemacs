...
#+END_SRC
*** minuet-ai
Use Minuet with the local Qwen 3.6 27B =llama-server= instance. The server is expected to expose an OpenAI-compatible chat endpoint at =http://127.0.0.1:8080/v1/chat/completions= with model alias =qwen3.6-27b=.

Override these defaults with =MINUET_QWEN_ENDPOINT=, =MINUET_QWEN_MODEL=, or =MINUET_QWEN_API_KEY= when needed.
#+BEGIN_SRC emacs-lisp :tangle user-config.el
  (use-package minuet
    ...
    :init
    (defun my/minuet-qwen-api-key ()
      "Return a placeholder API key for the local OpenAI-compatible Qwen server."
      (or (getenv "MINUET_QWEN_API_KEY") "local"))

    (add-hook 'prog-mode-hook #'minuet-auto-suggestion-mode)
    (add-hook 'git-commit-mode-hook #'minuet-auto-suggestion-mode)

    (spacemacs/declare-prefix "ai" "AI completion")
    ...
    (plist-put minuet-openai-compatible-options
               :end-point
               (or (getenv "MINUET_QWEN_ENDPOINT")
                   "http://127.0.0.1:8080/v1/chat/completions"))
    (plist-put minuet-openai-compatible-options :api-key #'my/minuet-qwen-api-key)
    (plist-put minuet-openai-compatible-options
               :model
               (or (getenv "MINUET_QWEN_MODEL") "qwen3.6-27b"))
    (plist-put minuet-openai-compatible-options
               :optional
               '(:max_tokens 128
                 :temperature 0.2
