...
#+BEGIN_SRC emacs-lisp :tangle user-config.el
  ...
  ;; Repeating async fetch: fires when Emacs has been idle for 30s, then repeats
  ;; every 30 minutes. org-gcal-fetch is non-blocking (async OAuth + HTTP).
  ;; Using idle timer ensures sync never interrupts active editing.
  (run-with-idle-timer 30 1800 #'my/org-gcal-fetch-safe)
