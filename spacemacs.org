...
#+END_SRC
*** minuet-ai
Use Minuet with an OpenAI-compatible completion endpoint. The default profile is IceBear =codex-proxy= at =http://127.0.0.1:8787/v1/chat/completions= with model =gpt-5.5= and low reasoning effort, using Codex Pro/ChatGPT quota through the local proxy. Run the proxy locally, log in through its dashboard, and export =CODEX_PROXY_API_KEY= with the dashboard API key.
    ...
    (when my/stl-review-preview-state
      (let ((source-path (plist-get my/stl-review-preview-state :source-path))
        ...
        (setq-local my/stl-review-source-path source-path
                    ...
                    scad-command (shell-quote-argument openscad))
          ...
          (progn
            (with-current-buffer adapter
              ...
              (when (bound-and-true-p flymake-mode)
                (flymake-mode -1))
              (let ((scad-command (shell-quote-argument openscad)))
                (if source-buffer
                    (scad-preview)
